...
public class Utils {
    ...
    public static String translateColorCodes(String string, Player player) {
        ...
        if (hasColorWarn) {
            player.sendMessage(Locales.NO_COLOR_PERMISSION.getComponent(player));
        }
        if (hasHexWarn) {
            player.sendMessage(Locales.NO_HEX_PERMISSION.getComponent(player));
        }

        if (Config.BLOCK_MAGIC_COLOR.getBoolean() && !player.hasPermission("chatex.chat.magic")) {
            if (containsMagicColor(result)) {
                player.sendMessage(Locales.MAGIC_BLOCKED.getComponent(player));
                shouldCancel = true;
    ...
    public static Component toComponent(String message) {
        ...
        if (message.contains("<") && message.contains(">")) {
            try {
                ...
                LogHelper.debug("MiniMessage parse failed in toComponent: " + e.getMessage());
            }
        }
    ...
    public static String replacePlayerPlaceholders(Player player, String format) {
        return replacePlayerPlaceholders(player, format, true);
    }

    public static String replacePlayerPlaceholders(Player player, String format, boolean parseColors) {
